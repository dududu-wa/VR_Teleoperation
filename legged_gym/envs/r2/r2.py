...
from legged_gym.envs.r2.r2_config import R2Cfg
...
class R2Robot(BaseTask):
    ...
    def _reward_standing_joint_deviation(self):
        if len(self.standing_joint_inds) == 0:
            return torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
        penalize_joint_index = torch.tensor(self.standing_joint_inds, dtype=torch.long, device=self.device)
    ...
    def _reward_joint_power_distribution(self):     
        # Penalize torques
        if len(self.knee_inds) == 0:
            return torch.zeros(self.num_envs, dtype=torch.float, device=self.device)
        shank_joint_index = torch.tensor(self.knee_inds, dtype=torch.long, device=self.device)
