...
- `R2Cfg`：定义 26 个控制 DOF、默认关节角、PD stiffness/damping/torque limits、trimesh 地形、gait/body 命令、奖励项、R2 URDF asset 路径、domain randomization。`commands.profile_mixture=None` 是默认关闭的 targeted ablation 开关，打开后由 `R2InterruptRobot._apply_command_profile_mixture()` 用一组带 jitter 的 profile 替换原矩形命令采样。`head_deviation=-1.0` 会启用头部回正惩罚；`shoulder_deviation=0` 会让 `R2Robot._prepare_reward_function()` 移除肩/手臂偏离默认姿态的奖励项，相当于关闭 PPO 训练里的肩膀回复/回正惩罚开关。
...
所以默认配置下干扰机制会实际启用：interrupt flag 写入 command，`DISTURB_DIM=10`，且 `use_disturb=True`。`disturb.start_by_curriculum=True` 会保留原始 curriculum-style release：noise-disturb 环境只有离开 terrain curriculum mode 后才开始更新/触发 disturb；消融 JSON 可设为 `false`，用于隔离固定 command range 时的 disturb release 影响。`disturb.staged_release=False` 是默认兼容开关；打开后，`stage_levels` 给出 disturb curriculum 上限序列，`stage_min_episodes`、`stage_min_task_return` 和 `stage_max_fall_rate` 决定是否从当前上限进入下一阶段，避免在 run 还没稳定时直接释放到满扰动。`stage_min_task_return` 和 `stage_max_fall_rate` 可保持 scalar，此时每个 stage 使用同一门槛；也可写成与 `stage_levels` 等长的 list，用于让早期 stage gate 更宽、后期逐步收紧。`stage_monitor_expert=None` 默认统计全部命令；设为 `run` / `walk` / `jump` 时，只用对应 AMP hard-route 语义的 episode 判断晋级。
`disturb.stage_init_curriculum_to_level=False` 是 July06 加入的 resume-only 开关。默认关闭时，`disturb_rad_curriculum` 仍从 0 开始，保持旧训练语义；显式打开时，`R2InterruptRobot.__init__()` 会用当前 staged cap 通过 `torch.full(...)` 初始化 `disturb_rad_curriculum`，让从高阶段 checkpoint 续训时不会重新从 0 扰动强度爬坡。这个设计只服务于后续阶段续训，避免改动从零训练的 curriculum 轨迹。
...
- `post_physics_step`：刷新状态，计算 base/foot/contact 状态，更新命令和地形高度，检查终止，计算奖励，reset 结束环境，重新计算观测；当评估脚本打开 `record_eval_pre_reset_state` 时，它会在 `reset_idx()` 前调用 `_cache_eval_pre_reset_state()` 保存终止前 root/rpy/velocity/contact/command 快照，避免 `evaluate.py` 在 `env.step()` 返回后只能看到 reset 后的初始姿态。
...
- `_cache_eval_pre_reset_state`：默认不运行，只服务于 `evaluate.py --record_state_trace`。它在 reset 前 clone 当前 `root_states`、`rpy`、`base_lin_vel`、`base_ang_vel`、`commands`、`contact_forces`、`reset_buf` 和 `time_out_buf`，让无头评估可以记录真实终止前状态，而不是 reset 后的 nominal base pose。

reward 约定：

- `_prepare_reward_function()` 会扫描 cfg 中非零 reward scale，并调用对应的 `_reward_<name>()`。
- `compute_reward()` 正常路径会把各 term 加到 `rew_buf` 和 `episode_sums`；当评估脚本显式设置 `record_reward_terms=True` 时，它还会把当前 step 的缩放后 reward term 复制到 `last_reward_terms`，用于 `evaluate.py --record_reward_terms` 在 reset 清空 `episode_sums` 前做逐 episode 诊断。该开关默认关闭，避免训练时每步 clone reward 张量。
...
- 显式传入 `--record_state_trace` 时，`evaluate.py` 会设置 `env.record_eval_pre_reset_state=True`，并用 `_init_state_trace_buffers()` 为每个 env 维护固定长度 tail window。`_append_state_trace()` 每步记录 base height、roll/pitch/yaw、base linear/yaw velocity、command、速度误差、最大 termination contact force 和 contact body；done env 会优先使用环境侧 reset 前快照。`_flush_state_trace_episode()` 只为请求的前 `--num_episodes` 个 episode 写出窗口样本，输出 `state_trace.json` / `state_trace.csv`，字段含 `steps_until_done`，用于定位 contact 或 orientation failure 是突然发生还是提前多步积累。
...
`Jun19` 的 `scratch_command_hold` 已实测跑通，命令如下：

```powershell
wsl.exe -d Ubuntu-22.04 --cd /mnt/e/codebase/VR_Teleoperation -- env PATH=/opt/miniconda3/envs/r2gym/bin:/opt/miniconda3/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin PYTHONPATH=/mnt/e/codebase/VR_Teleoperation:/mnt/e/codebase/VR_Teleoperation/rsl_rl LD_LIBRARY_PATH=/opt/miniconda3/envs/r2gym/lib:/mnt/e/wsl/isaacgym/isaacgym/python/isaacgym/_bindings/linux-x86_64 /opt/miniconda3/envs/r2gym/bin/python legged_gym/scripts/evaluate.py --task=r2amp --headless --sim_device=cpu --rl_device=cpu --num_envs=64 --load_run Jun19/Jun19_16-09-11_scratch_command_hold --checkpoint=-2 --cfg_override_json configs/ablation/scratch_command_hold.json --num_episodes=64 --episode_seconds=10 --output_dir outputs/eval/June19_command_hold_best

wsl.exe -d Ubuntu-22.04 --cd /mnt/e/codebase/VR_Teleoperation -- env PATH=/opt/miniconda3/envs/r2gym/bin:/opt/miniconda3/bin:/usr/local/sbin:/usr/local/bin:/usr/sbin:/usr/bin:/sbin:/bin PYTHONPATH=/mnt/e/codebase/VR_Teleoperation:/mnt/e/codebase/VR_Teleoperation/rsl_rl LD_LIBRARY_PATH=/opt/miniconda3/envs/r2gym/lib:/mnt/e/wsl/isaacgym/isaacgym/python/isaacgym/_bindings/linux-x86_64 /opt/miniconda3/envs/r2gym/bin/python legged_gym/scripts/evaluate.py --task=r2amp --headless --sim_device=cpu --rl_device=cpu --num_envs=64 --load_run Jun19/Jun19_16-09-11_scratch_command_hold --checkpoint=8000 --cfg_override_json configs/ablation/scratch_command_hold.json --num_episodes=64 --episode_seconds=10 --output_dir outputs/eval/June19_command_hold_8000
```

关键参数说明：

- `--load_run Jun19/<run_dir>`：`get_load_path()` 的根目录是 `logs/r2_amp`，所以实际日志子目录 `Jun19` 必须写进 `--load_run`。
